...
import static com.example11.database.MyDatabaseHelper.DATABASE_NAME;
...
public class DBManager {
    ...
    private static DBManager map;
    private static Context mContext;
    private SQLiteDatabase db;

    //写数据时调用
    public static DBManager getInstance(Context context,Class clazz) {
        if (map == null) {
            map = new DBManager(context,clazz);
        }
        mContext = context;
        return map;
    }
    ...
    public DBManager(Context context,Class clazz) {
        ...
        if(db == null){
            db = MyDatabaseHelper.getInstance(context,clazz).getWritableDatabase();
        }
    }
    ...
    /**
     *使用：Person person = new Person("Tom",18,false);
